...
`;var a={$schema:"https://acode.app/schema/plugin/v0.1.0.json",id:"acode.plugin.pyscript",name:"PyScript",main:"src/main.js",version:"1.0.0",repository:"https://github.com/azzammuhyala/pyscript",icon:"PyScript.png",minVersionCode:290,license:"MIT",keywords:["pyscript","pyslang","pys","programming","language","programming language","highlight"],price:0,permissions:[],author:{name:"Azzam Muhyala",email:"azzammuhyala@gmail.com",github:"azzammuhyala"}};var C=acode.require("url"),F=acode.require("aceModes"),u=ace.require("ace/snippets").snippetManager,m=class{async init(n,t,E){t.id=a.id,ace.define("ace/mode/pyscript",function(c,A,j){let x=c("ace/lib/oop"),f=c("ace/mode/text").Mode,R=c("ace/mode/text_highlight_rules").TextHighlightRules,y=function(){this.$rules=W,this.normalizeRules()};x.inherits(y,R);let h=function(){f.call(this),this.HighlightRules=y;let l=this.getTokenizer(),T=l.getLineTokens.bind(l),P=["string_apostrophe_single","string_quotation_single","raw_string_apostrophe_single","raw_string_quotation_single"];l.getLineTokens=function(b,q){let g=T(b,q);return P.includes(g.state)&&!b.endsWith("\\")&&(g.state="start"),g}};x.inherits(h,f),A.Mode=h}),F.addMode("pyscript",v,"PyScript");let $=n+"PyScript.png";acode.addIcon("pyscript-icon",$),this.iconStyle=document.createElement("style"),this.iconStyle.textContent=`
        .file_type_pyscript::before {
            ...
            height: 1em;
            width: 1em;
        }`,document.head.append(this.iconStyle),this.snippet=u.parseSnippetFile(S),u.register(this.snippet,"pyscript"),editorManager.editor.setOptions({enableBasicAutocompletion:!0,enableSnippets:!0,enableLiveAutocompletion:!0}),this.updateSession("pyscript")}async destroy(){this.iconStyle.remove(),u.unregister(this.snippet,"pyscript"),this.updateSession("text"),F.removeMode("pyscript")}applyPluginToFile(n){C.extname(n.name)===".pys"&&n.session.setMode("ace/mode/pyscript")}updateSession(n){for(let t of editorManager.files)!t||!t.name||this.applyPluginToFile(t);editorManager.emit("update")}},U=async()=>{for(;!window.acode||!window.acode.setPluginInit||!window.editorManager||!window.acode.require||document.readyState==="loading";)await new Promise(o=>setTimeout(o,250))};window.acode&&U().then(()=>{let o=new m;acode.setPluginInit(a.id,async(n,t,E)=>{await o.init(n.endsWith("/")?n:n+"/",t,E)}),acode.setPluginUnmount(a.id,()=>{o.destroy()})});})();
